...
async function selectCoin(coin) {
  ...
  try {
    const res = await fetch(
      `${COINGECKO_BASE}/simple/price?ids=${encodeURIComponent(coin.id)}&vs_currencies=usd&include_24hr_change=true`
    );
    ...
    resultPrice.textContent = `$${priceData.usd.toLocaleString(undefined, { maximumFractionDigits: 8 })}`;
